...
export default function ProfilePage() {
        ...
        if (profileData.role === "CREATOR" || profileData.role === "ADMIN") {
          try {
            const mangasRes = await fetch(
              `${API_URL}/mangas/creator/${profileData.id}`,
              { headers: { Authorization: `Bearer ${token}` } }
            );
            if (mangasRes.ok) {
              const mangasData = await mangasRes.json();
              mangasList = mangasData.data || [];
            }
          } catch (err) {
  ...
  const handleDeleteManga = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/mangas/${mangaToDelete.id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      setProfile((prev) => {
        ...
        return {
          ...prev,
          mangas: (prev.mangas || []).filter((m: any) => m.id !== mangaToDelete.id),
          _count: {
            ...prev._count,
            mangas: Math.max(0, (prev._count?.mangas || 1) - 1),
          },
        };
      });
    ...
    <>
      <div className="flex flex-col min-h-screen pb-24 bg-background text-foreground selection:bg-blue-500/30 selection:text-foreground">
        ...
        <main className="max-w-4xl mx-auto w-full px-4 md:px-8 -mt-14 md:-mt-20 flex flex-col items-center">
          ...
          {!isAdmin && <div className="w-full max-w-md mb-4">{renderCreatorButton()}</div>}
                  ...
                  {profile.mangas.map((manga: any) => (
                    <div key={manga.id} className="group relative">
                      <Link
                        ...
                        className="block relative aspect-[2/3] bg-muted rounded-lg overflow-hidden border border-border/60 hover:scale-[1.02] hover:border-blue-500/50 transition-all duration-200"
                      >
                      ...
                      <button
                        ...
                        className="absolute top-1.5 right-1.5 p-1.5 rounded-full bg-rose-600/90 hover:bg-rose-500 text-white opacity-0 group-hover:opacity-100 transition-all shadow-lg z-10"
          ...
          <div className="w-full max-w-md bg-card border border-border/80 rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div className="px-5 py-5 space-y-3">
              <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/40 border border-border/60">
                {mangaToDelete.coverUrl ? (
                  <img
                    src={mangaToDelete.coverUrl}
                    alt={mangaToDelete.title}
                    className="w-14 h-20 rounded-lg object-cover border border-border/60 shrink-0"
                  />
                ) : (
                  <div className="w-14 h-20 rounded-lg bg-card border border-border/60 flex items-center justify-center shrink-0">
            ...
            <div className="flex gap-3 px-5 py-4 border-t border-border/60 bg-muted/20">
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2.5 rounded-xl bg-card border border-border text-foreground hover:bg-muted text-sm font-medium transition-all disabled:opacity-50"
              >
                Annuler
              </button>
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-sm font-bold transition-all shadow-lg shadow-rose-600/20 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {deleting ? (
                  <>
